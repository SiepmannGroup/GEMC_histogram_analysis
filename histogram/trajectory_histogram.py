...
class Trajectory(Blocks):
    ...
    def initialize_data(self):
        ...
        self.total_number_of_molecules = sum(
            all_data['N'][i][box][1] for i in all_data['N'].keys() for box in all_data['V'].keys()
        )
        self.num_molecules = {box: [] for box in self.box_volumes}
        for box in self.boxes:
            for i in all_data['N']['molecule 1'][box].keys():
        ...
        self.total_volume = sum(
            all_data['V'][box][1] for box in all_data['V'].keys()
        )  # nm**3
        self.box_volumes = {box:
                                [x / self.total_volume for x in self.box_volumes[box].values()]
                            for box in all_data['V'].keys()}
        self.num_molecules = {box:
                                  [x / self.total_number_of_molecules for x in self.num_molecules[box]]
                              for box in all_data['V'].keys()}
    ...
    def plot_probability_histogram(self):
        ...
        hist, x_edges, y_edges, im = ax.hist2d(
            Vi_over_Vt, Ni_over_Nt, bins=self.number_of_bins, density=True,
            range=[[0., 1.], [0., 1.]],
